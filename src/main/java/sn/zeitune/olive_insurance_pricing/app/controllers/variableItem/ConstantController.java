...
public class ConstantController {
    ...
    @GetMapping
    public ResponseEntity<Page<ConstantResponseDTO>> readAll(
            @PageableDefault(size = 20) Pageable pageable,
            Authentication authentication
    ) {
        log.info("REST request to get all constants with pagination: page={}, size={}", pageable.getPageNumber(), pageable.getPageSize());
        Page<ConstantResponseDTO> response = constantService.retrieveAllActive(((Employee) authentication.getPrincipal()).getManagementEntity(), pageable);
